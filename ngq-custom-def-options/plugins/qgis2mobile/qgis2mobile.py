...
class Plugin():
  def __init__(self, iface):
    ...
    self.systemPluginPath = QgsApplication.prefixPath() + "/python/plugins/qgis2mobile"
    ...
    if QFileInfo(self.userPluginPath).exists():
      ...
      translationPath = self.systemPluginPath + "/i18n/qgis2mobile_" + localeFullName + ".qm"
  ...
  def initGui(self):
    icon_path = self.userPluginPath + "/icons/qgis2mobile.png"
    if not QFileInfo(icon_path).exists():
        icon_path = self.systemPluginPath + "/icons/qgis2mobile.png"
        
    self.actionRun = QAction(self.iface.mainWindow())
    self.actionRun.setIcon(QIcon(icon_path))
    ...
    self.actionSettings.setIcon(QIcon(icon_path))
